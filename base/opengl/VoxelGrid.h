...
namespace ct {
	namespace vao {
		namespace VoxelGrid {
			...
			static void init_vao(VAO &vao, Eigen::MatrixXf &vertices, Eigen::MatrixXf &normals, Eigen::MatrixXf &positions, Eigen::MatrixXf &colors, std::string shader_dir) {
				...
				GLuint vs = 0, fs = 0;
				oglh::load_shader(vs, shader_dir + "/VoxelGridVS.glsl", GL_VERTEX_SHADER);
				oglh::load_shader(fs, shader_dir + "/VoxelGridFS.glsl", GL_FRAGMENT_SHADER);
				GLuint shaders[2] = {vs, fs};
				oglh::create_program(vao.id_program, shaders, 2);
