...
namespace ct {
	namespace vao {
		namespace VoxelGrid {
			...
			static void init_vao(VAO &vao, Eigen::MatrixXf &vertices, Eigen::MatrixXf &normals, Eigen::MatrixXf &positions, Eigen::MatrixXf &colors, float scale, std::string shader_dir) {
			...
			static void draw(VAO& vao, int n_size, Eigen::Matrix4f &model_matrix, Eigen::Matrix4f &view_matrix, Eigen::Matrix4f &projection_matrix) {
				glUseProgram(vao.id_program);
				glBindVertexArray(vao.id_vao); 
